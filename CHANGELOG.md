...
### Added
- `tests/config-needs-tool.py`, run by the CI gate: every config directory the install links must belong to a tool the repo installs, guards, or declares a prerequisite with a reason. An undeclared config dir fails, so nobody can ship config for an uninstalled tool silently.
...
### Added
- `bat` is now installed and updated like the other tools: apt where available, GitHub binary otherwise, with a `bat` shim when apt supplies `batcat`. `test.sh` checks it, and the no-sudo profile checks it strictly in `~/.local/bin`.
- `test.sh` now also checks tmux strictly in `~/.local/bin` for the no-sudo profile, covering the installer added in 1.12.0.
...
### Fixed
- nvim rendered near-black on near-black on terminals without truecolor (#53). Such terminals now get `habamax` and `notermguicolors`; 256-colour terminals are unchanged. Force the old behaviour in `~/.config/nvim/local.lua`.
...
### Added
- CI: an Ubuntu 16.04 cell (git 2.7.4) exercises init.lua's full-clone fallback, the only path where `--filter` is unavailable. It asserts git rejects `--filter` first, since git 2.17 accepts it and a version check would pass while testing nothing.

## [1.11.4] - 2026-09-17

### Fixed
- zsh: plugin clones no longer abort the install on bash <= 4.3 (Ubuntu 16.04), where `"${arr[@]}"` on an empty array is an unbound variable under `set -u`. With 1.11.3's apt fix, install.sh now completes on 16.04.
...
- `modules/base.sh`, `update.sh`: regenerate `~/.fzf.zsh` when missing on
  ...
  whenever `~/.fzf/` existed, so a machine that lost `~/.fzf.zsh` (or never
  had it generated by an older code path) silently had no fzf shell
