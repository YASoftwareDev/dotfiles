...
### Fixed
- git: `merge.conflictstyle` is `diff3` in the tracked config; `zdiff3` made git < 2.35 (Ubuntu 20.04/22.04) abort every checkout and merge, which also left lazy.nvim plugins on their default branch. Re-run install.sh to restore `zdiff3` via `~/.gitconfig.local` where git supports it.
